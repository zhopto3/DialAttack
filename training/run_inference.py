...
def get_args():
    ...
    parser.add_argument(
        "--task",
        choices=["asr", "dial_class", "adversarial"],
        default="asr",
    ...
    parser.add_argument(
        "--batch_size", default=64, type=int, help="Inference batch size"
    )
...
def main():
    ...
    test_loader = DataLoader(
        test_set, shuffle=False, batch_size=args.batch_size, collate_fn=dynamic_padding
    )
    ...
    evaluator = Inference(
        task=args.task,
        model=network,
        ckpt_path=f"./{args.experiment_name}/checkpoints/{args.checkpoint_name}",
