...
class Trainer:
    ...
    def train(self, initial_lr, es_patience, lr_patience, logger_level = logging.INFO):
        ...
        logging.basicConfig(filename=f'{self.name}.log',filemode="w", format='%(asctime)s %(message)s')
    ...
    def _save_best_ckpt(self):
        os.makedirs(f'../{self.name}_ckpts',exist_ok=True)
        if os.path.isfile(f'../{self.name}_ckpts/best_checkpoint.pt'):
            os.remove(f'../{self.name}_ckpts/best_checkpoint.pt')
        ...
        },f'../{self.name}_ckpts/best_checkpoint.pt')