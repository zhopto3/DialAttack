        ...
        with torch.no_grad():
            for x, t, dial, audio_l, txt_l, path in self.eval_loader:
                ...
                decoded_gold = self.tokenizer.decode(
                    t[0].detach().cpu().type(torch.int64).tolist()
                )
                wer = error_rate(decoded_gold, decoded_model, char=False)

                print(
                    f"{self.dial_codes[dial[0]]}\t{path[0]}\t{decoded_gold}\t{decoded_model}\t{wer}"
                )
        ...
        with torch.no_grad():
            for x, t, dialect, audio_l, txt_l in self.eval_loader:
                ...
                for gold_seq, model_seq, dial in zip(t, out_seqs, dialect):
                    ...
                    decoded_gold = self.tokenizer.decode(
                        gold_seq.detach().cpu().type(torch.int64).tolist()
                    )
                    decoded_model = self.tokenizer.decode(
                        model_seq.detach().cpu().type(torch.int64).tolist()
                    )
                    ...
                    print(
                        f"{self.dial_codes[dial]}\t{decoded_gold}\t{decoded_model}\t{wer}\t{cer}"
                    )
