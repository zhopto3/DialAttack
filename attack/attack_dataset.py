...
class AttackCV(torchaudio.datasets.COMMONVOICE):
    ...
    def __init__(
        ...
        super(AttackCV, self).__init__(
            ...
            tsv=(
                "attack_samp_balanced.tsv"
                if not (blackbox)
                else f"attack_samp_{experiment}.tsv"
            ),
        )
        if blackbox:
    ...
    def __getitem__(self, n: int) -> Tuple[Tensor, list, int]:
        ...
        if samp_rate != self.model_samp_rate:
            waveform = torchaudio.functional.resample(
                waveform, orig_freq=samp_rate, new_freq=self.model_samp_rate
            )

        return (
            waveform,
            self.encoded_attack,
            self.dial.get(metadata["grouped_accents"]),
            metadata["path"],
        )
