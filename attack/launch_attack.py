...
def get_args():
    parser = argparse.ArgumentParser(
        "Script to launch CW adversarial attacks on fine-tuned ASR models"
    )
    ...
    parser.add_argument(
        "--k", type=int, default=8, help="Max number of times to attenuate the eta"
    )
...
def main(args):
    ...
    dataset = AttackCV(
        bundle.sample_rate,
        attack_target="Porta'm a un lloc web malvat.",
        vocab=args.vocab_path,
        path=args.data_path,
    )

    data_loader = DataLoader(
        dataset, shuffle=False, batch_size=1, collate_fn=attack_collator
    )
    ...
    attacker = CWAttack(
        ...
        dial_codes=dataset.code_2_dial,
